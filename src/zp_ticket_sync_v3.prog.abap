...
START-OF-SELECTION.
  CASE abap_true.
    ...
    WHEN p_polic.
      DATA(police_sys) = NEW zcl_ticket_police_sys( ).
      remote_sys ?= police_sys.

    WHEN p_artif.
      DATA(artif_sys) = NEW zcl_ticket_ai_sys( ).
      remote_sys ?= artif_sys.

    WHEN p_arti2.
      DATA(arti2_sys) = NEW zcl_ticket_ai_trg_sys( ).
      remote_sys ?= arti2_sys.
